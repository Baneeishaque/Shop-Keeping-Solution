...
public class Common_Tasks {
    ...
    public static String return_current_date_time() {
        return LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy  hh:mm a"));
    }

    public static String return_sum_of_a_part_from_combo_box_collection_items_seperated_by_a_symbol(JComboBox purchase_numbers, String symbol, int part_positon) {
        Double total = 0.0;
        for (int i = 0; i < purchase_numbers.getItemCount(); i++) {
            String[] collection = purchase_numbers.getItemAt(i).toString().split(symbol);
//            for (int j = 0; j < collection.length; j++) {
//                Common_Tasks.debug_display(collection[j]);
//            }
            total = total + Double.parseDouble(collection[part_positon]);

        }
        return String.valueOf(total);
    }

    public static String return_part_of_a_string_seperated_by_a_symbol(String raw, String symbol, int position) {
    ...
    public static void populate_combobox_by_parsing_string_with_symbol(JComboBox jc, String raw, String symbol) {
        ...
        for (int i = 1; i < collection.length; i++) {
            debug_display(collection[i]);
            if (!is_combo_box_contains_an_item(jc, collection[i])) {
                jc.addItem(collection[i]);
            }
        }

    }

    public static boolean is_combo_box_contains_an_item(JComboBox jc, String Item) {
        for (int i = 0; i < jc.getItemCount(); i++) {
            if (jc.getItemAt(i).toString().equals(Item)) {
                return true;
            }
        }
        return false;
    ...
    public static void populate_combobox_from_vector(JComboBox jc, Vector v) {
        jc.removeAllItems();
    ...
    public static void add_blank_row_to_table(JTable jt) {
        DefaultTableModel m = (DefaultTableModel) jt.getModel();
    ...
    public static String return_total_of_a_column_from_jTable(JTable jt, int Column_number) {
        ...
        for (int i = 0; i < jt.getRowCount(); i++) {
            if ((!jt.getValueAt(i, 0).toString().isEmpty()) && (!jt.getValueAt(i, Column_number).toString().isEmpty())) {
...
//            Common_Tasks.add_single_row_to_table(jTable_purchase_bill, new String[]{String.valueOf(i + 1), Common_Tasks.parse_string_seperated_by_full_column(table_data.elementAt(i).toString()).elementAt(0).toString(), Common_Tasks.parse_string_seperated_by_full_column(table_data.elementAt(i).toString()).elementAt(1).toString(), Common_Tasks.parse_string_seperated_by_full_column(table_data.elementAt(i).toString()).elementAt(2).toString(), Common_Tasks.parse_string_seperated_by_full_column(table_data.elementAt(i).toString()).elementAt(3).toString(), Common_Tasks.parse_string_seperated_by_full_column(table_data.elementAt(i).toString()).elementAt(4).toString(), Common_Tasks.parse_string_seperated_by_full_column(table_data.elementAt(i).toString()).elementAt(5).toString()});
    ...
    public static void debug_display(Object raw) {
        System.out.println(raw);
    }
    ...
    public static void remove_rows_from_table(JTable jt) {
